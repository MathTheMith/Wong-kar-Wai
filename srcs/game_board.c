...
void update_horizontal(t_board *board, int (*game_board)[board->size][board->size], int move, int *has_moved)
{
    int j_start;
    int last_j;
    j_start = 1;
    if (move == -1)
        j_start = board->size - 2;
    
    for (int i = 0; i < board->size; i++)
    {
        for (int j = j_start; 0 <= j && j < board->size; j += move)
        {
            last_j = j;
...
void update_vertical(t_board *board, int (*game_board)[board->size][board->size], int move, int *has_moved)
{
    int i_start;
    int last_i;
    ...
    i_start = 1;
    if (move == -1)
        i_start = board->size - 2;
    
    for (int j = 0; j < board->size; j++)
    {
        for (int i = i_start; 0 <= i && i < board->size; i += move)
        {
            last_i = i;
...
void update_game_board(t_board *board, int (*game_board)[board->size][board->size], int ch)
{
    ...
    {
        case KEY_LEFT:
            update_horizontal(board, game_board, 1, &has_moved);
            break;
        ...
        case KEY_UP:
            update_vertical(board, game_board, 1, &has_moved);
            break;
    ...
    if (has_moved)
        spawn_rand(board->size, game_board, 1);
